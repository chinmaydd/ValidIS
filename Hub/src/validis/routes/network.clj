...
(def network-routes
  ...
  (context "/api/network" []
    ...
    (PATCH "/:id/:network-id" {:as request}
          :path-params [id :- String network-id :- String]
          :body-params [{name :- String ""} {location :- String ""}]
          :header-params [authorization :- String]
          ...
          :description "Authorization header expects the following format 'Token {token}'"
          (modify-network-response request name location id network-id))

    (DELETE "/:id/:network-id" {:as request}
            :path-params [id :- String network-id :- String]
            :header-params [authorization :- String]
            ...
            :description "Authorization header expects the following format 'Token {token}'"
            (delete-network-response request id network-id))))
